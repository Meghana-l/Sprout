// Vercel serverless function (CommonJS). Route: POST /api/generate
module.exports = async (req, res) => {
  ...
  try {
    // Body may arrive unparsed depending on runtime — handle both.
    let body = req.body;
    if (!body || typeof body === "string") {
      try { body = JSON.parse(body || "{}"); } catch (e) { body = {}; }
    }
    const { notes, tone = "Warm", length = "Standard", lang = "English", milestone = true } = body || {};
    ...
    if (!r.ok) return res.status(502).json({ error: "Groq: " + (data && data.error && data.error.message ? data.error.message : JSON.stringify(data)) });

    const text = (data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) || "";
    const clean = text.replace(/```json/gi, "").replace(/```/g, "").trim();
    let parsed;
    try { parsed = JSON.parse(clean); }
    catch (e) { return res.status(502).json({ error: "Could not parse model output: " + text.slice(0, 180) }); }
